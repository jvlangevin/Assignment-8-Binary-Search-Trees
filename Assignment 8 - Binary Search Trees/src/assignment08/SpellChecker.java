...
public class SpellChecker
{
	...
	/**
	 * Returns a list of the words contained in the specified file. (Note that symbols, digits, and spaces are ignored.)
	 * 
	 * @param file
	 *            - the File to be read
	...
	private List<String> readFromFile(File file)
	{
		...
		{
			/*
			 * Java's Scanner class is a simple lexer for Strings and primitive types (see the Java API, if you are unfamiliar).
			 */

			/*
			 * The scanner can be directed how to delimit (or divide) the input. By default, it uses whitespace as the delimiter. The following statement specifies anything other than alphabetic
			 * characters as a delimiter (so that punctuation and such will be ignored). The string argument is a regular expression that specifies "anything but an alphabetic character". You need not
